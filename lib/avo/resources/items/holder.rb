class Avo::Resources::Items::Holder
  ...
  def field(field_name, **args, &)
    ...
    if from.present? && from.class == Avo::Resources::Items::TabGroup::Builder
      return from.field(field_name, holder: self, **args, &)
    end

    field_parser = Avo::Dsl::FieldParser.new(id: field_name, order_index: @items_index, **args, &).parse
  ...
  def tabs(tab = nil, id: nil, name: nil, **, &)
    if tab.present?
      add_item tab
    else
      add_item Avo::Resources::Items::TabGroup::Builder.parse_block(parent: @parent, id: id, name: name, **, &)
    end
  end

  def tab(name, **args, &)
    add_item Avo::Resources::Items::Tab::Builder.parse_block(name: name, parent: @parent, **args, &)
  end

  def row(**args, &)
    add_item Avo::Resources::Items::Row::Builder.parse_block(parent: @parent, **args, &)
  end
  ...
  def panel(panel_name = nil, **args, &)
    add_item Avo::Resources::Items::ItemGroup::Builder.parse_block(name: panel_name, parent: @parent, **args, &)
  end

  # The main panel is the one that also render the header of the resource with the breadcrumbs, the title and the controls.
  def main_panel(**args, &)
    add_item Avo::Resources::Items::MainPanel::Builder.parse_block(name: "main_panel", parent: @parent, **args, &)
  end

  def sidebar(**args, &)
    check_sidebar_is_inside_a_panel

    add_item Avo::Resources::Items::Sidebar::Builder.parse_block(parent: @parent, **args, &)
  end
