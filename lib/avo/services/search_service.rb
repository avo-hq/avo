module Avo
  module Services
    class SearchService
      ...
      option :id_attribute, default: -> { :_id }
      option :label_attribute, default: -> { :_label }
      option :mark_labels, default: -> { true }
      ...
      def search_resource(resource)
        ...
        [resource.route_key, result_object]
      end
      ...
      def fetch_result_information(record, resource, item)
        ...
        highlighted_title = if mark_labels
          highlight(title&.to_s, params[:q])
        else
          title&.to_s
        end
        ...
        {
          id_attribute => record.id,
          label_attribute => highlighted_title,
