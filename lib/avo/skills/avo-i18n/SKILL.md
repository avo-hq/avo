...
`description` fills the resource's panel description and, per the cascade, beats `self.description` on the class. The beat is unconditional: if `self.description` is a block reading `record` or `view` (a per-record or per-view description), the translation still replaces it wholesale — don't add the key for a resource that needs that dynamic text.
