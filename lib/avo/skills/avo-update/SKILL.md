...
Guide sections are headed by version (`## Upgrade to 3.22.0`, `## Upgrade from 3.16.2 to 3.16.3`) or by the change itself when a version only shipped one. Some are per-add-on, naming the gem and its version (an "Upgrade to avo-dashboards 4.0.9" heading), and they're interleaved with core's sections in version order rather than grouped at the end. **A bare version heading does not mean the section is about core** — the paid gems release on a shared train and take core's version, so a plain "Upgrade to 4.2.0" can describe an add-on's breaking change (4.2.0's four are all `avo-api`'s). Read each section's content against your whole lock diff, not just the core row, and don't skip a version because core didn't cross it. A version section may hold **several named changes**, each with its own "Action required" note — work them all, not just the first. A `## Unreleased — …` section at the very top describes changes that have landed on `main` but aren't in a released gem yet; skip it unless the app tracks Avo from git. Newest is at the top — **read up, then apply down.**
...
- **An upgrade section can require a generator and a migration.** A gem that shipped no table before may ship one now, and its resource can appear in the sidebar the moment the gem is bundled — before the table exists, so the screen errors until you migrate. Run the **upgrade** generator the section names, not the install one: `avo-api` 4.2.0 wants `rails generate avo_api:tokens` (idempotent, leaves your controllers alone), while `avo_api:install` also re-runs `avo_api:generate` and offers to overwrite every resource controller you've customized. This is the one case where a migration is expected — it doesn't license guessing at one the guide doesn't name.
