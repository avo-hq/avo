...
3. If it applies, make the change — which can be a generator plus a migration rather than a code edit — then boot the app and re-run the tests.
...
- **A section can ask for a generator and a migration, not an edit.** `4.2.0` is the first: `avo-api` grew a tokens table, so the step is `rails generate avo_api:tokens` then `rails db:migrate`, and until both have run the API tokens screen — and any request carrying an `Authorization: Bearer` header — queries a table that isn't there. Run the **narrow** generator the section names, never the gem's `install` one on an app that already has it: `avo_api:install` also re-runs `avo_api:generate` and offers to overwrite every resource controller the app has customized.
