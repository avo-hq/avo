...
- **Editing the resource or policy file, or upgrading Avo or a plugin, *does* bust the cache automatically** — `file_hash` (part of the default `cache_hash`) hashes both files plus `Avo.cache_version`, a digest of the installed Avo and plugin versions. So config changes take effect immediately; only *data*/*association* changes need the fixes above.
