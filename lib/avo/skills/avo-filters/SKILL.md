...
- **`encode_filters` only applies filters the resource declares.** Filter state travels in the `encoded_filters` param and outlives the code that wrote it, so Avo keeps only the classes listed in that resource's `def filters` and discards the rest with a log warning. A link naming a filter the resource does not declare loads the index with that filter ignored — check `def filters` before blaming the payload.
