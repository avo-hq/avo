...
- **Only filters the resource currently declares are applied.** Filter state (an `encoded_filters` link, or a persisted session) outlives the code that wrote it, so if a filter is later removed from `def filters` — or its class deleted outright — Avo discards it with a log warning instead of erroring. Someone holding an old link or session just gets the index without that filter applied; check `def filters` and the logs before blaming the payload.
