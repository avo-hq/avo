...
Avo also resolves `description` below the resource's translation key — `avo.resource_translations.user.description` supplies the User resource description. Per the cascade, the locale file **wins** and `self.description` is the fallback, so an app can keep an English default in code and translate over it — **avo-i18n**. That win is unconditional: if `self.description` is a block reading `record`/`view` for per-record or per-view text, a resolved translation still replaces it wholesale, so skip the locale key for a resource that needs the dynamic block.
...
| `self.color` | Tint for that icon (palette name) | `self.color = :purple` |
