...
- **`icon:` isn't universal.** It works on `section` and individual items, **not** on `group` or on sub-items nested inside a `resource` or `link_to` block. (`link_to` takes a block for sub-items the same way `resource` does — the parent link stays clickable. The DSL itself is **avo-menu**'s skill.)
