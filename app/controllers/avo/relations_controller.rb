  ...
  class RelationsController < BaseController
    ...
    def create
      ...
      respond_to do |format|
        if @model.save
          format.html { redirect_to resource_path(@model, for_resource: @related_resource), notice: t("avo.attachment_class_attached", attachment_class: @attachment_class) }
          format.json { render :show, status: :created, location: resource_path(@model, for_resource: @related_resource) }
        else
    ...
    def destroy
      ...
      respond_to do |format|
        format.html { redirect_to params[:referrer] || resource_path(@model, for_resource: @related_resource), notice: t("avo.attachment_class_detached", attachment_class: @attachment_class) }
      end
    end
