...
RSpec.feature 'belongs_to', type: :feature do
  ...
  context 'index' do
    ...
    subject { visit url; find("[data-resource-id='#{post.id}'] [data-field-id='user']") }
  ...
  context 'show' do
    ...
    describe 'with user attached' do
      ...
      it { is_expected.to have_link admin.name, href: "/avo/resources/users/#{admin.id}?via_resource_id=#{post.id}&via_resource_name=posts" }
    end
  ...
  context 'edit' do
    ...
    describe 'without user attached' do
      ...
      it 'changes the user' do
        ...
        expect(page).to have_link admin.name, href: "/avo/resources/users/#{admin.id}?via_resource_id=#{post.id}&via_resource_name=posts"
      end
    end
    ...
    describe 'with user attached' do
      ...
      it 'changes the user' do
        ...
        expect(current_path).to eql "/avo/resources/posts/#{post.id}"
        expect(page).to have_link second_user.name, href: "/avo/resources/users/#{second_user.id}?via_resource_id=#{post.id}&via_resource_name=posts"
      end
