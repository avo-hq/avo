...
RSpec.describe "Multiple resources on same model", type: :feature do
  ...
  describe 'each resource' do
    it "returns to parent resource when cancel creation" do
      ...
      expect(page).to have_current_path "/admin/resources/posts/new?association_name=posts&via_record_id=#{user.to_param}&via_relation=user&via_relation_class=User&via_resource_class=Avo%3A%3AResources%3A%3ACompactUser"
