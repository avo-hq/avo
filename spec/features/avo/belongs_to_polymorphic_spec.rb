      ...
      context "without an association" do
        context "when not filling the poly association" do
          it "creates the comment" do
            ...
            expect(find_field_value_element("user")).to have_link user.name, href: "/admin/resources/compact_users/#{user.slug}?related_name=user&via_record_id=#{Comment.last.id}&via_resource_class=Avo%3A%3AResources%3A%3AComment"
