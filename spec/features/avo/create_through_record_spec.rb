...
RSpec.feature "create_through_record", type: :feature do
  ...
  describe "belongs_to comment" do
    let(:url) { "/admin/resources/comments/new?via_relation=user&via_relation_class=User&via_resource_id=#{admin.id}" }
  ...
  describe "has_many" do
    ...
    let(:url) { "/admin/resources/spouses/new?via_relation=spouses&via_relation_class=Person&via_resource_id=#{person.to_param}" }
    ...
    it "attaches the spouse to a person" do
      ...
      expect(current_path).to eq "/admin/resources/people/#{person.to_param}"
    end
  end
  ...
  describe "has_many through" do
    ...
    let(:url) { "/admin/resources/users/new?via_relation=teams&via_relation_class=Team&via_resource_id=#{team.id}" }
  ...
  describe "has_and_belongs_to_many" do
    let(:url) { "/admin/resources/projects/new?via_relation=users&via_relation_class=User&via_resource_id=#{admin.id}" }
  ...
  describe "polymorphic" do
    ...
    let(:url) { "/admin/resources/comments/new?via_relation=commentable&via_relation_class=Project&via_resource_id=#{project.id}" }
