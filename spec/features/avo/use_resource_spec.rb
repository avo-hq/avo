...
RSpec.describe "Post comments use_resource PhotoComment", type: :feature do
  ...
  describe "tests" do
    ...
    it "if create new photo comment persist and attach" do
      ...
      expect(page).to have_current_path "/admin/resources/photo_comments/new?via_record_id=#{post.slug}&via_relation=commentable&via_relation_class=Post&via_resource_class=Post"
    ...
    it "applies on belongs to" do
      ...
      expect(page).to have_link comment.user.name,
        href: "/admin/resources/compact_users/#{comment.user.slug}?via_record_id=#{comment.to_param}&via_resource_class=Comment"

      click_on comment.user.name
      expect(page).to have_current_path "/admin/resources/compact_users/#{comment.user.slug}?via_record_id=#{comment.to_param}&via_resource_class=Comment"
