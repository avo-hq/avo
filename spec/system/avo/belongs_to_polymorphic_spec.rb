        ...
        context "when filling the poly association" do
          describe "creating a polymorphic association" do
            it "creates the comment" do
              ...
              expect(page).to have_link post.name, href: "/admin/resources/posts/#{post.slug}?related_name=commentable&via_record_id=#{Comment.last.id}&via_resource_class=Avo%3A%3AResources%3A%3AComment"
    ...
    describe "within a parent model" do
      ...
      context "in a project show page" do
        it "has the comment listed" do
          ...
          expect(find_field_value_element("user")).to have_link user.name, href: "/admin/resources/compact_users/#{user.slug}?related_name=user&via_record_id=#{comment.id}&via_resource_class=Avo%3A%3AResources%3A%3AComment"
          expect(find_field_value_element("commentable")).to have_link project.name, href: "/admin/resources/projects/#{project.id}?related_name=commentable&via_record_id=#{comment.id}&via_resource_class=Avo%3A%3AResources%3A%3AComment"
