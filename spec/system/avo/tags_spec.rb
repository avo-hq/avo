...
RSpec.describe 'Tags', type: :system do
  describe 'acts_as_taggable' do
    ...
    context 'show' do
      ...
      it 'shows filled state' do
        post.tag_list = %w[some tags here and there]
        post.save
    ...
    context 'edit' do
      ...
      it 'adds a tag' do
        ...
        expect(post.reload.tag_list.sort).to eq %w[1 2].sort
      end
    end
    ...
    context 'new' do
      it 'keeps acts as taggable tags on validation errors' do
        ...
        expect(page.all('.tagify__tag-text').map(&:text)).to eq %w[one two]
      end
    end
  end
  ...
  describe 'without acts_as_taggable' do
    ...
    it 'adds skills' do
      ...
      expect(course.reload.skills.sort).to eq %w[some skills one two three].sort
    end
  end
    ...
    it 'on index' do
      Avo::Resources::Project.with_temporary_items do
        ...
        field :dummy_field, as: :tags,
                            ...
                            decorate: -> { [{ value: 'a_c2', label: 'activity_category2' }] },
                            suggestions: [
