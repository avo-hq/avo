...
RSpec.describe "Attachments", type: :request do
  ...
  it "denies has_one_attached upload when upload_cover? is false" do
    sign_in admin_user
    ...
    allow_any_instance_of(Avo::Services::AuthorizationService)
      ...
      .with("upload_cover?", record: post_record, raise_exception: false)
      .and_return(false)

    upload = build_upload(content: "new", original_filename: "new.txt", content_type: "text/plain")
    old_blob_id = post_record.reload.cover.blob_id
    old_checksum = post_record.cover.blob.checksum

    expect {
      post "/admin/avo_api/resources/posts/#{post_record.to_param}/attachments",
        params: {file: upload, filename: "new.txt", attachment_key: "cover"},
        headers: {"ACCEPT" => "application/json"}
    }.not_to change { ActiveStorage::Blob.count }

    post_record.reload
    expect(post_record.cover).to be_attached
    expect(post_record.cover.blob_id).to eq(old_blob_id)
    expect(post_record.cover.blob.checksum).to eq(old_checksum)
  ...
  describe "DELETE destroy" do
    ...
    it "does not destroy an attachment belonging to another record" do
      attach(post_record, :cover, "mine")
      victim = attach(other_post, :cover, "victim")

      expect {
        destroy_attachment(post_record, "cover", victim.id)
      }.not_to change { ActiveStorage::Attachment.exists?(victim.id) }.from(true)

      expect(other_post.reload.cover).to be_attached
    end
    ...
    it "does not destroy anything when the attachment name is not an attachment on the record" do
      mine = attach(post_record, :cover, "mine")
    ...
    it "destroys the attachment when it belongs to the record and the named association" do
      ...
      expect {
        destroy_attachment(post_record, "cover", mine.id)
      }.to change { ActiveStorage::Attachment.exists?(mine.id) }.from(true).to(false)

      expect(post_record.reload.cover).not_to be_attached
    end
    ...
    it "does not destroy the attachment when the delete policy denies it" do
      ...
      allow_any_instance_of(Avo::Services::AuthorizationService)
        .to receive(:authorize_action)
        .with("delete_cover?", record: post_record, raise_exception: false)
        .and_return(false)
